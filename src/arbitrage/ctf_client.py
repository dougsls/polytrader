...
⚠️ Web3 sync (run_in_executor) — a lib `web3` 7.x ainda é síncrona em
seu hot path. Como o copy-engine, encapsulamos em executor pra não
...
class CTFClient:
    ...
    def _ensure_initialized(self) -> None:
        ...
        # Polygon usa POA — precisa do middleware.
        try:
    ...
    async def merge_binary(self, condition_id: str, size: float) -> str:
        ...
        return await loop.run_in_executor(None, self._merge_sync, condition_id, size)

    def _merge_sync(self, condition_id: str, size: float) -> str:
        from web3 import Web3  # type: ignore[import-not-found]
        ...
        usdc = Web3.to_checksum_address(self._usdc_address)
        owner = Web3.to_checksum_address(self._account_address)

        fn = self._contract.functions.mergePositions(usdc, parent, cond, partition, amount_units)

        nonce = self._w3.eth.get_transaction_count(owner, "pending")
        # Gas: estima e adiciona 20% de margem; cap em 500k pra evitar
        # estimativas absurdas (CTF mergePositions tipicamente ~150k gas).
        try:
            est = fn.estimate_gas({"from": owner})
            gas = min(int(est * 1.2), 500_000)
        except Exception as exc:  # noqa: BLE001
            log.warning("ctf_gas_estimate_failed", err=repr(exc))
            gas = 350_000  # safe fallback

        # EIP-1559 fields — Polygon tem fee market.
        try:
            base = self._w3.eth.get_block("latest")["baseFeePerGas"]
            max_priority = self._w3.to_wei(30, "gwei")
        ...
        except (KeyError, Exception):  # noqa: BLE001 — fallback legacy
        ...
        log.info(
            "ctf_merge_submitted",
            tx=tx_hash_hex, cond_id=condition_id, size=size, gas=gas,
        )
        # Espera confirmação — bloqueia até receipt.
        receipt = self._w3.eth.wait_for_transaction_receipt(
            tx_hash, timeout=_TX_RECEIPT_TIMEOUT_S,
        )
        if receipt.status != 1:
            raise RuntimeError(f"CTF merge tx failed on-chain: {tx_hash_hex}")
        log.info("ctf_merge_confirmed", tx=tx_hash_hex, gas_used=receipt.gasUsed)
        return tx_hash_hex
    ...
    async def redeem(self, condition_id: str, index_sets: list[int]) -> str:
        ...
        loop = asyncio.get_running_loop()
        return await loop.run_in_executor(None, self._redeem_sync, condition_id, index_sets)

    def _redeem_sync(self, condition_id: str, index_sets: list[int]) -> str:
        from web3 import Web3  # type: ignore[import-not-found]
        ...
        if receipt.status != 1:
            raise RuntimeError(f"CTF redeem tx failed: {tx_hash_hex}")
        return tx_hash_hex
    ...
    async def position_balance(self, token_id: str) -> float:
        ...
        """
        loop = asyncio.get_running_loop()
        return await loop.run_in_executor(None, self._balance_sync, token_id)

    def _balance_sync(self, token_id: str) -> float:
        from web3 import Web3  # type: ignore[import-not-found]
        ...
        raw = self._contract.functions.balanceOf(owner, tid).call()
