    ...
    async with get_connection(db_path) as db:
        ...
        if signal.side == "BUY":
            if row is None:
                ...
                wallets = orjson.loads(row["source_wallets_json"])
                ...
                await db.execute(
                    ...
                    (new_size, new_avg, executed_price,
                     orjson.dumps(wallets).decode(), now, row["id"]),
                )
        else:  # SELL
            if row is None:
            ...
            if new_size <= _EPS:
                await db.execute(
                    ...
                    (now, now, executed_price, row["id"]),
                )
            else:
                await db.execute(
                    "UPDATE bot_positions SET size=?, current_price=?, updated_at=? "
                    "WHERE id=?",
                    (new_size, executed_price, now, row["id"]),
                )
